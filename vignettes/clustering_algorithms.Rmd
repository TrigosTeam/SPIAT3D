...
Check out the image below, I'm not lying, each circle only touches two points.
Image from [link](https://doc.cgal.org/latest/Alpha_shapes_2/). Points enclosed
between the circles are part of the alpha hull itself.
```{r}
# Showing nice image to describe alpha hulls
knitr::include_graphics(system.file("extdata", "alphashape.png", package = "SPIAT3D"))
```

This applies to 2D and can be extended to 3D by using spheres instead. The
radius of the sphere (i.e. the alpha value) can be determined by the user. A
small alpha value can lead to more clusters with more intricate borders (but may
also miss clusters as a result), while a large alpha value can result in fewer, 
...
that collectively define your cluster.
...
knitr::include_graphics(system.file("extdata", "border_of_clusters.png", package = "SPIAT3D"))
```
The threshold used is the median number of cluster cells found in all the
spheres, as this provied a robust threshold value.
...
# several plots, only the last plot matters?
...
estimate for the centre of each cluster.
```{r}
# Using dbscan clusters
