...
in, draw a sphere around then. Tally the number of cells inside each sphere and
...
knitr::include_graphics(system.file("extdata", "cell_colocalization.png", package = "SPIAT3D"))
```
...
seeing if the proportion of interacting reference and target cells is similar/
balanced (~50:50) around each reference cell.
...
knitr::include_graphics(system.file("extdata", "neighbourhood_entropy.png", package = "SPIAT3D"))
```
...
I've combined these two metrics, because their both outputted by the same
function.
...
For a given sphere radius, a COO metric is calculated by determining the 
proportion of target cells found within the sphere of each reference cell, and 
