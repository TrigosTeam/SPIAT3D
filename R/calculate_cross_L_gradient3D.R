calculate_cross_L_gradient3D <- function(spe, 
  ...
  colnames(result) <- c("observed_cross_L", 
                        "expected_cross_L",
                        "cross_L_ratio")
  ...
  if (plot_image) {
    fig1 <- plot_cross_L_gradient3D(result, reference_cell_type, target_cell_type)
    fig2 <- plot_cross_L_gradient_ratio3D(result, reference_cell_type, target_cell_type)
